...
export default function Navigation({ isLogged, name, email }: { isLogged: boolean; name?: string; email?: string }) {
  return (
    <Disclosure as="header" className="bg-white shadow mb-2">
      {({ open }) => (
        <>
            ...
            <div className="relative flex h-16 justify-between">
              ...
              {isLogged && (
                <div className="hidden lg:relative lg:z-10 lg:ml-4 lg:flex lg:items-center">
                  <button
                    type="button"
                    className="flex-shrink-0 rounded-full bg-white p-1 text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                  >
                  ...
                  <Menu as="div" className="relative ml-4 flex-shrink-0">
                    <div>
                      <Menu.Button className="flex rounded-sm bg-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 flex-col">
                        <span className="sr-only">Open user menu</span>
                        <span className="text-sm">{name}</span>
                        <span className="underline text-xs">{email}</span>
                      </Menu.Button>
                    </div>
                    <Transition
                      as={Fragment}
          ...
          <Disclosure.Panel as="nav" className="lg:hidden" aria-label="Global">
            {/* mobile navigation here -> if needed  */}
